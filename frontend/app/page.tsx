...
export default function App() {
  ...
  const executeCommand = useCallback(async (cmdId:string, args?:string)=>{
    ...
    if(cmdId==="ajuda"){ const help=`**🛠️ Comandos P15 REAL + P24 Brainstorming**\n\n${CHAT_COMMANDS.map(c=>`**${c.label}** - ${c.desc}`).join("\n\n")}\n\n**⌨️** Ctrl+K foco, Ctrl+L limpa, Ctrl+B workplace, Ctrl+J agentes\n\n**🧠 P24 Brainstorming Deep Integration:** Auto brainstorm quando detecta construir app — cria app, landing, dashboard, ecommerce, portfolio, saas, chat app, blog, api — 3-5 abordagens com pros/cons: MVP 70% 5min, Fullstack 90% 30min, Custom 95% 1-2h — 20 templates P22: chat-rag, saas-auth, portfolio-blog, ecommerce-ai, dashboard-analytics, landing-ai, api-webhook — BrainstormPanel clicável Escolher esta → cria projeto com template — Você no centro, terceiro olho\n\n**🚀 Export REAL P15:** GitHub API se GITHUB_TOKEN set, Vercel se VERCEL_TOKEN set, Docker se docker available`; setChatMessages(p=>[...p,{role:"assistant",content:help}]); return; }
    ...
    try{ const last=[...chatMessages].reverse().find(m=>m.role==="assistant"); const data=await api.post("/api/commands/execute",{command:cmdId,args:args||"",project_id:selectedProject?.project_id||null,context:last?.content?.slice(0,2000)||""}); const content=`**${cmd.label} - ${data.description}**\n\n**Agentes:** ${data.agents_triggered.join(", ")}\n\n**Crítica:** ${data.critical_analysis}\n\n\`\`\`json\n${JSON.stringify(data.result,null,2).slice(0,2000)}\n\`\`\`\n\n**Próximos:** ${data.next_steps.join(" → ")}`; setChatMessages(p=>[...p,{role:"assistant",content,commandResult:data, brainstorm_panel: data.result?.brainstorm_panel || data.brainstorm_panel || null}]); }catch(e:any){ setChatMessages(p=>[...p,{role:"assistant",content:`❌ Erro: ${e.message}`,isError:true}]); }
  }, [chatMessages, projects, selectedProject]);

  const createFromTemplate = useCallback(async (templateId:string, customPrompt?:string)=>{
    try{
      const data=await api.post(`/api/projects/templates/${templateId}/create`);
      setProjects((prev:any)=>[data,...prev]);
      setSelectedProject(data);
      setSelectedFile(Object.keys(data.files)[0]||"");
      setEditingFile(data.files[Object.keys(data.files)[0]||""]||"");
      setWorkplaceOpen(true);
      setChatMessages(p=>[...p,{role:"assistant",content:`✅ Projeto criado com template **${templateId}** — ${data.name} — ${Object.keys(data.files).length} files — ${customPrompt ? `Prompt: ${customPrompt.slice(0,200)}` : ""}\n\nVocê no centro, agora customiza para objetivo final. Workplace aberto.`, meta:{selected:`template ${templateId}`, profile:"TEMPLATE"}}]);
    }catch(e:any){ setChatMessages(p=>[...p,{role:"assistant",content:`❌ Erro criar template ${templateId}: ${e.message}`,isError:true}]); }
  }, [setProjects]);
  ...
  const sendChat = useCallback(async ()=>{
    ...
    try{
      ...
      else if(streamEnabled){ const reader=res.body?.getReader(); const decoder=new TextDecoder(); let assistantContent=""; let assistantMeta:any=null; let assistantBrainstorm:any=null; setChatMessages(p=>[...p,{role:"assistant",content:"",meta:null, brainstorm_panel:null}]); if(reader){ while(true){ const {done,value}=await reader.read(); if(done)break; const chunk=decoder.decode(value); for(const line of chunk.split("\n")){ if(line.startsWith("data: ")){ const dataStr=line.slice(6); if(dataStr==="[DONE]")break; try{ const data=JSON.parse(dataStr); if(data.choices?.[0]?.delta?.content){ assistantContent+=data.choices[0].delta.content; setChatMessages(prev=>{const n=[...prev]; n[n.length-1]={...n[n.length-1],content:assistantContent}; return n;}); } if(data.routing){ assistantMeta=data.routing; setChatMessages(prev=>{const n=[...prev]; n[n.length-1]={...n[n.length-1],meta:data.routing, brainstorm_panel: data.brainstorm_panel || n[n.length-1].brainstorm_panel}; return n;}); } if(data.brainstorm_panel){ assistantBrainstorm=data.brainstorm_panel; setChatMessages(prev=>{const n=[...prev]; n[n.length-1]={...n[n.length-1],brainstorm_panel:data.brainstorm_panel}; return n;}); } }catch(e){} } } } } // final fetch for brainstorm if not streamed
        try{
          // After streaming done, try to get brainstorm from a follow-up non-stream call? Actually backend streams only content, not brainstorm_panel — need to handle via final chunk
          // For now, if we have content but no brainstorm, check if last message should have brainstorm — backend adds brainstorm_panel to openai_resp even when streaming? We already handle data.routing.brainstorm_panel via final chunk
        }catch{}
      }
      else{ const data=await res.json(); setChatMessages(p=>[...p,{role:"assistant",content:data.choices?.[0]?.message?.content||"Sem resposta",meta:data.routing||data.observability, brainstorm_panel:data.brainstorm_panel||null, brainstorm:data.brainstorm||null}]); }
